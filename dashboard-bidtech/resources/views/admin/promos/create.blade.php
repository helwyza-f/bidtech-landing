...
@section('title', 'Buat Kode Promo - Dashboard Admin Bidtech')
...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Buat Kode Promo</h1>
            <p class="text-sm text-slate-500 mt-1">Isi formulir di bawah untuk membuat kode voucher atau diskon baru.</p>
        </div>
    </div>

    <form action="{{ route('dashboard.promos.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" placeholder="Contoh: HEMAT50"
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Promo Hemat Spesial"
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="Deskripsi singkat promo ini..."
                          class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 p-3 rounded-xl bg-emerald-50 border border-emerald-100">
                ...
                <label for="is_active" class="text-sm font-semibold text-emerald-800 cursor-pointer">Aktifkan promo ini segera setelah dibuat</label>
            </div>
        </div>

        {{-- Konfigurasi Diskon & Harga --}}
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-5">
            <h2 class="text-base font-bold text-slate-800 flex items-center gap-2">
            ...
            <div id="promo-type-info" class="p-3.5 rounded-xl bg-violet-50 border border-violet-100 text-xs text-violet-800 flex items-start gap-2.5 transition">
                ...
                <div id="promo-type-desc">
                    <strong>Tipe Promo:</strong> Pilih tipe promo dan cakupan di bawah untuk melihat simulasi cara kerja perhitungan.
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="type" id="promo_type" onchange="updatePromoFormHelper()"
                        ...
                        <option value="fixed" {{ old('type') === 'fixed' ? 'selected' : '' }}>Nominal Tetap (Potongan Rp)</option>
                        <option value="percentage" {{ old('type') === 'percentage' ? 'selected' : '' }}>Persentase (Potongan %)</option>
                        <option value="override_price" {{ old('type') === 'override_price' ? 'selected' : '' }}>Harga Pasti / Fixed Price (Patokan Harga)</option>
                        <option value="free" {{ old('type') === 'free' ? 'selected' : '' }}>Gratis Total Paket (Rp0)</option>
                        <option value="free_component" {{ old('type') === 'free_component' ? 'selected' : '' }}>Gratis Komponen Tertentu (Rp0)</option>
                        <option value="bundle_price" {{ old('type') === 'bundle_price' ? 'selected' : '' }}>Harga Bundle Flat (Rp)</option>
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" inputmode="numeric" name="reward_amount" id="reward_amount" 
                           value="{{ old('reward_amount') ? (is_numeric(str_replace('.', '', old('reward_amount'))) ? number_format((int)str_replace('.', '', old('reward_amount')), 0, ',', '.') : old('reward_amount')) : '' }}" 
                ...
                <div id="max_discount_container">
                    ...
                    <input type="text" inputmode="numeric" name="max_discount" id="max_discount" 
                           value="{{ old('max_discount') ? number_format((int)str_replace('.', '', old('max_discount')), 0, ',', '.') : '' }}" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="target_scope" id="target_scope" onchange="updatePromoFormHelper()"
                        ...
                        <option value="all" {{ old('target_scope', 'all') === 'all' ? 'selected' : '' }}>Semua Komponen Paket (General / Subtotal)</option>
                        <option value="service" {{ old('target_scope') === 'service' ? 'selected' : '' }}>Hanya Layanan / Setup (Services)</option>
                        <option value="template" {{ old('target_scope') === 'template' ? 'selected' : '' }}>Hanya Biaya Template</option>
                        <option value="server" {{ old('target_scope') === 'server' ? 'selected' : '' }}>Hanya Biaya Server</option>
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" inputmode="numeric" name="min_order_amount" id="min_order_amount" 
                           value="{{ old('min_order_amount') ? number_format((int)str_replace('.', '', old('min_order_amount')), 0, ',', '.') : '' }}" 
                ...
                <div id="min_applicable_container">
                    ...
                    <input type="text" inputmode="numeric" name="min_applicable_price" id="min_applicable_price" 
                           value="{{ old('min_applicable_price') ? number_format((int)str_replace('.', '', old('min_applicable_price')), 0, ',', '.') : '' }}" 
            ...
            <div>
                ...
                <select name="target_template_id" class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 text-slate-700">
                    ...
                    @foreach($templates as $tpl)
                        <option value="{{ $tpl->id }}" {{ old('target_template_id') == $tpl->id ? 'selected' : '' }}>{{ $tpl->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" min="1" placeholder="Kosong = tidak terbatas"
                           class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Batas Per User</label>
                    <input type="number" name="usage_per_user" value="{{ old('usage_per_user') }}" min="1" placeholder="Kosong = tidak terbatas"
                           class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-5">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" id="is_partner" name="is_partner" value="1" {{ old('is_partner') ? 'checked' : '' }}
            ...
            <div id="partner-fields" class="{{ old('is_partner') ? '' : 'hidden' }} space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Nama Mitra</label>
                        <input type="text" name="partner_name" value="{{ old('partner_name') }}" placeholder="Nama komunitas/instansi mitra"
                               class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Kode Referral Mitra</label>
                        <input type="text" name="partner_code" value="{{ old('partner_code') }}" placeholder="Kode internal mitra"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="partner_commission_type" class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 text-slate-700">
                            ...
                            <option value="fixed" {{ old('partner_commission_type') === 'fixed' ? 'selected' : '' }}>Nominal Tetap (Rp)</option>
                            <option value="percentage" {{ old('partner_commission_type') === 'percentage' ? 'selected' : '' }}>Persentase (%)</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="text" inputmode="numeric" name="partner_commission_value" id="partner_commission_value" 
                               value="{{ old('partner_commission_value') ? number_format((int)str_replace('.', '', old('partner_commission_value')), 0, ',', '.') : '' }}" 
                               placeholder="0"
                               class="rupiah-input w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="allowed_domains" value="{{ old('allowed_domains') }}" placeholder="polibatam.ac.id, batam.go.id"
                               class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="allowed_emails" value="{{ old('allowed_emails') }}" placeholder="john@gmail.com, jane@yahoo.com"
                               class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500">
                        <p class="text-[11px] text-slate-400 mt-1">Pisahkan dengan koma</p>
                    </div>
                </div>
